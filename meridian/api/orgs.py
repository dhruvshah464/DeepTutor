...
from meridian.persistence.models.org import Invitation, Organization, OrgMembership, OrgRole
    ...
    member_count = (
        await db.execute(
            select(func.count(OrgMembership.id)).where(
                OrgMembership.org_id == org_id, OrgMembership.is_active.is_(True)
            )
        )
    ).scalar() or 0
...
async def list_members(
    ...
    from meridian.persistence.models.user import User, UserProfile
...
from meridian.persistence.models.user import User  # noqa: E402
