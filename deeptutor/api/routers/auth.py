...
from deeptutor.auth.jwt import (
...
from deeptutor.auth.schemas import (
    LoginRequest,
    MagicLinkRequest,
...
from deeptutor.database.models.org import OrgMembership, Organization
from deeptutor.database.models.user import User, UserPreferences, UserProfile
...
async def login(
    ...
    membership = (
        await db.execute(
            ...
            .where(OrgMembership.user_id == user.id, OrgMembership.is_active == True)
            .limit(1)
        )
    ).scalar_one_or_none()
    if membership:
